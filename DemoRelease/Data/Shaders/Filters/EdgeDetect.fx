...
// TODO : bool gbEdgeColourOverride  // [ELExxxUI_BOOL:Edge Colour Override]   (Currently we show the source colour on edges, but we should offer being able to use single colour (as a separate param)
//     --- float4 gEdgeOverrideColour = float4(1,1,1,1);    // [ELEUI_COLOUR:Colour]
...
float4 EdgePS(VS_OUTPUT input) : COLOR
{
    ...
    // Threshold for edge
    float edge = edgeStrength > gEdgeThreshold ? 1.0 : 0.0;

    // Output edge as white, background as black
    return float4(edge * c11.rgb, 1.0);
}

technique CannyEdgeApprox
{
    pass P0
    {
